...
################################################################################
# This file contains code for simulations in the homoscedastic scenario of     #
#                                                                              #
...
# Last revised on: 31/01/2024                                                  #
#                                                                              #
################################################################################



## All settings are to be specified in PART 0 below (lines 23-150),
# rest of the code needs not be altered

## Settings in this file assume: sigma_T is homosk., exp. (any combinations
# true-test copula possible: dep-dep / dep-indep / indep - dep / indep-indep)

############ PART 0: SETTINGS SPECIFICATION ###################################

### external storage
folder_path <- paste0("/home/remote/Myrthe/","Papersim_scen1_hom_cvg_2024_01_31")
...
test_cop_name = "clayton"
test_lambda = 0.5


### data generation settings
true_cop_name = "clayton" # if "indep", put true_ktau = 0
true_ktau <- 0.5
...
# true_distr_T <- "expWb"
# 
# # only for generating from expWeibull distribution, otherwise comment
# true_wb_shape = 1.5
# true_wb_scale = 2
# 
# # true distribution for C (always normal)
...
## Homoskedasticity for T ###
sigma_fun_hom <- function(gam){
  return(exp(gam))
}
...
determine_init_basis_hom <- function(T1,X1,C0,X0,Delta){
  ...
  beta_intercept <- as.numeric(quantile(T_res_no_intercept, test_lambda))
  ...
  sigma_T_init <- sqrt(var(T_res_no_intercept) *
                         (test_lambda^2*(1 - test_lambda)^2)/(test_lambda^2 + (1-test_lambda)^2))
  ...
  for (j in 1:init_value_number_basis){
    ...
    init_values[[j]] <- c(eta_init, perturb(beta_init_update, init_seeds[3] + j),
                          perturb(gamma_T_init, init_seeds[4] + j),
...
### (ii) summary function for transformations on all parameters
para_inv_transform_hom <- function(transf_para_vec_hom, cop_name){
  para_vec <- transf_para_vec_hom
...
dEAL_hom <- function(y, x, beta, gam_par, lambda, phi_til, phi){
  s <- sigma_fun_hom(gam_par)
  z <- 1/s * (y - x %*% beta)
  ...
  if (length(y) == 1){# avoid indexing one vector x as x[pos_indices,]
    ...
    result[pos_indices] <- lambda*(1-lambda)/s*1/sqnorm(phi_ext)*
      exp(-z_pos*lambda) * right_sum^2
    result[-pos_indices] <- (1-lambda)*lambda/s*1/sqnorm(phi_til_ext)*
      exp(-z_neg *(lambda-1)) * left_sum^2
  }
  return(as.matrix(result))
}

pEAL_neg_hom <- function(y, x, beta, gam_par, lambda, phi_til){
...
pEAL_pos_hom <- function(y, x, beta, gam_par, lambda, phi){
...
pEAL_hom <- function(y, x, beta, gam_par, lambda, phi_til, phi){
  s <- sigma_fun_hom(gam_par)
  z <- 1/s * (y - x %*% beta)
  ...
  if (length(y) == 1){# avoid indexing one vector x as x[pos_indices,]
    if (z < 0){
      res_vec <- pEAL_neg_hom(y, x, beta, gam_par, lambda, phi_til)
    } else {
      res_vec <- pEAL_pos_hom(y, x, beta, gam_par, lambda, phi)
    }
    ...
    if (length(pos_indices) == 0){
      ...
      res_vec[pos_indices,] <- pEAL_pos_hom(y[pos_indices], x[pos_indices,],
                                            beta, gam_par, lambda, phi)
      
      
      if (length(pos_indices) < length(z)){
        res_vec[-pos_indices,] <- pEAL_neg_hom(y[-pos_indices], x[-pos_indices,],
...
qEAL_hom <- function(w, x, beta, gam_par, lambda, phi_til, phi){
  ...
  f_null <- function(y){
    return(pEAL_hom(y, x_null, beta_null, gam_par_null, lambda, phi_til, phi) - w)
  }
...
log_lik_fun_hom <- function(all_paras, lag_degs, lambda, cop_name, Y, X, Delta){
  
  # 1) Retrieving current parameters & corresponding copula object ----
  
  paras_T_indices <- c(beta_full_index, gamma_index, phi_til_index(lag_degs),
  ...
  ## density
  dens_T_un <- dEAL_hom(Y_un, X_un, beta_full, gam_par, lambda, phi_til, phi)
  dens_C_cens <- dCens(Y_cens, X_cens, transf_paras_C)
  
  ## distribution
  distr_T_un <- pEAL_hom(Y_un, X_un, beta_full, gam_par, lambda, phi_til, phi)
  distr_T_cens <- pEAL_hom(Y_cens, X_cens, beta_full, gam_par, lambda, phi_til, phi)
...
neg_log_lik_fun_hom <- function(all_paras, lag_degs, lambda, cop_name, Y, X, Delta){
...
ctuity_constr_fun_hom <- function(all_paras, lag_degs,
                                  lambda, cop_name, Y, X, Delta){
...
perform_mll_optimisation_hom <- function(lag_degs, lambda, cop_name, Y, X, Delta,
                                         init_val_list,
  ...
  if (opt_algo == "NM"){
    if (indep_assumption){
      for (init_vals in init_val_list){
        res <- optim(init_vals[-1], log_lik_fun_indep_hom, method = "Nelder-Mead",
      ...
      for (init_vals in init_val_list){
        res <- optim(init_vals, log_lik_fun_hom, method = "Nelder-Mead",
                     control = list(fnscale = -1, maxit = it_num_unconstr),
                     lag_degs = lag_degs,
                     lambda = lambda,
                     cop_name = cop_name,
  ...
  if (opt_algo == "NMCob"){
    ...
    if (indep_assumption){
      for (init_vals in init_val_list){
        intermed_para <- optim(init_vals[-1], log_lik_fun_indep_hom,
                               method = "Nelder-Mead",
        ...
        res <- nloptr(x0 = intermed_para,
                      eval_f = neg_log_lik_fun_indep_hom, # max. rather than min.!
                      eval_g_ineq = ctuity_constr_fun_indep_hom,
                      opts = opts,
                      lag_degs = lag_degs,
                      lambda = lambda,
                      cop_name = cop_name,
      ...
      for (init_vals in init_val_list){
        intermed_para <- optim(init_vals, log_lik_fun_hom,
                               method = "Nelder-Mead",
                               control = list(fnscale = -1,
                                              maxit = it_num_unconstr),
                               lag_degs = lag_degs,
        ...
        res <- nloptr(x0 = intermed_para,
                      eval_f = neg_log_lik_fun_hom, # max. rather than min.!
                      eval_g_ineq = ctuity_constr_fun_hom,
                      opts = opts,
                      lag_degs = lag_degs,
                      lambda = lambda,
                      cop_name = cop_name,
...
# para_variable and para_fixed together contain all parameters EXCEPT lambda
log_lik_partial_hom <- function(para_variable, para_fixed, fixed_indices,
                                lag_degs, lambda, cop_name, Y, X, Delta){
  ...
  return(log_lik_fun_hom(all_paras, lag_degs, lambda, cop_name, Y, X, Delta))
}

neg_log_lik_partial_hom <- function(para_variable, para_fixed, fixed_indices,
                                    lag_degs, lambda, cop_name, Y, X, Delta){
  all_paras <- rep(0, length(para_variable) + length(para_fixed))
  all_paras[fixed_indices] <- para_fixed
  all_paras[-fixed_indices] <- para_variable
...
ctuity_constr_partial_hom <- function(para_variable, para_fixed, fixed_indices,
                                      lag_degs, lambda, cop_name, Y, X, Delta){
...
perform_mll_partial_optimisation_hom <- function(lag_degs, lambda, cop_name, Y, X, Delta,
  ...
  if (opt_algo == "NM"){
    if (indep_assumption){
      for (init_vals in init_values_variable){
        res <- optim(init_vals[-1], log_lik_indep_partial_hom, method = "Nelder-Mead",
                     control = list(fnscale = -1, maxit = it_num_unconstr),
                     para_fixed = para_fixed, fixed_indices = fixed_indices,
                     lag_degs = lag_degs, lambda = lambda, cop_name = cop_name,
                     Y = Y, X = X, Delta = Delta)
      ...
      for (init_vals in init_values_variable){
        res <- optim(init_vals, log_lik_partial_hom, method = "Nelder-Mead",
                     control = list(fnscale = -1, maxit = it_num_unconstr),
                     para_fixed = para_fixed, fixed_indices = fixed_indices,
                     lag_degs = lag_degs, lambda = lambda, cop_name = cop_name,
                     Y = Y, X = X, Delta = Delta)
  ...
  if (opt_algo == "NMCob"){
    ...
    if (indep_assumption){
      for (init_vals in init_values_variable){
        intermed_para_variable <- optim(init_vals[-1], log_lik_indep_partial_hom,
                                        method = "Nelder-Mead",
                                        control = list(fnscale = -1,
                                                       maxit = it_num_unconstr),
        ...
        res <- nloptr(x0 = intermed_para_variable,
                      eval_f = neg_log_lik_indep_partial_hom, # max. rather than min.!
                      eval_g_ineq = ctuity_constr_partial_hom,
                      opts = opts,
                      para_fixed = para_fixed, fixed_indices = fixed_indices,
                      lag_degs = lag_degs, lambda = lambda, cop_name = cop_name,
                      Y = Y, X = X, Delta = Delta)
      ...
      for (init_vals in init_values_variable){
        intermed_para_variable <- optim(init_vals, log_lik_partial_hom,
                                        method = "Nelder-Mead",
                                        control = list(fnscale = -1,
                                                       maxit = it_num_unconstr),
                                        para_fixed = para_fixed,
        ...
        res <- nloptr(x0 = intermed_para_variable,
                      eval_f = neg_log_lik_partial_hom, # max. rather than min.!
                      eval_g_ineq = ctuity_constr_partial_hom,
                      opts = opts,
                      para_fixed = para_fixed, fixed_indices = fixed_indices,
                      lag_degs = lag_degs, lambda = lambda, cop_name = cop_name,
...
# para_variable and para_fixed together contain all parameters EXCEPT
# lambda AND ktau
log_lik_indep_partial_hom <- function(para_variable, para_fixed, fixed_indices,
                                      lag_degs, lambda, cop_name, Y, X, Delta){
  all_paras <- rep(0, length(para_variable) + length(para_fixed))
  all_paras[fixed_indices] <- para_fixed
  all_paras[-fixed_indices] <- para_variable
  return(log_lik_fun_indep_hom(all_paras, lag_degs, lambda,
                               cop_name, Y, X, Delta))
}

neg_log_lik_indep_partial_hom <- function(para_variable, para_fixed, fixed_indices,
                                          lag_degs, lambda, cop_name, Y, X, Delta){
  all_paras <- rep(0, length(para_variable) + length(para_fixed))
  all_paras[fixed_indices] <- para_fixed
  all_paras[-fixed_indices] <- para_variable
...
log_lik_fun_indep_hom <- function(all_paras_indep, lag_degs, lambda,
                                  cop_name, Y, X, Delta){
  
  # 1) Retrieving current parameters ----
  paras_T_indices <- c(beta_full_index, gamma_index,
                       phi_til_index(lag_degs), phi_index(lag_degs))
  # no eta present, so no shift + 1
  
  transf_paras_T <- all_paras_indep[paras_T_indices]
  transf_paras_C <- all_paras_indep[-paras_T_indices] # no ktau present at all
...
# exactly the same as previous function, but now returning minus the result
neg_log_lik_fun_indep_hom <- function(all_paras_indep, lag_degs, lambda,
                                      cop_name, Y, X, Delta){
...
ctuity_constr_fun_indep_hom <- function(all_paras_indep, lag_degs, lambda,
                                        cop_name, Y, X, Delta){
  # args. on second line not needed, but should be the same as for obj. fun.
...
deg_sel_AIC_grid_only_T_hom <- function(cop_name, Y, X, Delta,
                                        T1, X1, C0, X0,
                                        lambda,
                                        indep_assumption = FALSE,
  ...
  init_val_basis <- determine_init_basis_hom(T1,X1,C0,X0,Delta)
  mll_basis <- perform_mll_optimisation_hom(c(0,0), lambda = lambda,
                                            test_cop_name,
                                            Y, X, Delta,
  ...
  T_indices <- ktau_length_test + c(beta_full_index, gamma_index)
      ...
      if (L_degree + R_degree > 0){
        ...
        for (j in 1:init_value_number_intermed){
          ...
          if (ktau_length_test == 1){
            ...
            init_val_variable[[j]] <- c(0, slight_perturb(
              para_T_basis,(total_degree + 1)*(init_seeds[3] + j)),random_phis)
          }
        }
        
        new_mll <- perform_mll_partial_optimisation_hom(
...
para_performance_summary_hom <- function(ext_print = FALSE,
                                         ext_boxplots = FALSE){
  ####### Valid in general ######################
  ...
  if (true_distr_T == "EAL"){
    # in this case, (possibly) phis are present in the true para
    ...
    para_names_adjusted <- c("ktau", "beta_0", beta_T_all, "gamma_T",
                             phi_til_all, phi_all,
    ...
    if (true_diff_left > 0){
      ...
      if (true_left_deg == 0){
        append_index <- gamma_index + ktau_length_true
      } else {
    ...
    if (true_diff_right > 0){
      ...
      if (true_right_deg == 0){
        append_index <- gamma_index + max_overall_left_deg + ktau_length_true
      } else {
    ...
    max_para_num <- 2*X_dimension + 4 + 1 +
    ...
    for (sim_ind in seq_len(simulation_size)){
      ...
      if (sel_diff_left > 0){
        ...
        if (sel_left_deg == 0){
          append_index <- gamma_index + ktau_length_test
        } else {
      ...
      if (sel_diff_right > 0){
        ...
        if (sel_right_deg == 0){
          append_index <- gamma_index + max_overall_left_deg  + ktau_length_test
        } else {
          append_index <- max(phi_index(
    ...
    non_present_indices <- max(gamma_index) + ktau_length_true +
      seq_len(max_left_degree + max_right_degree)
    ...
    for (sim_ind in seq_len(simulation_size)){
      ...
      if (sel_diff_right > 0){
        ...
        if (sel_right_deg == 0){
          append_index <- gamma_index + max_left_degree  + ktau_length_test
        } else {
          append_index <- max(phi_index(
  ...
  if (ext_print){
    ...
    df_parameters <- as.data.frame(
      ...
      row.names = c("True parameters", "Mean estimates", "Variance", "RMSE",
                    "Bias", "Bias (rel)"))
    colnames(df_parameters) <- para_names_adjusted
    
    para_summary_file <-  paste0(folder_path, "/para_summary.txt")
    print.char.matrix(df_parameters, file = para_summary_file, cell.align = "cen",
  ...
  if (ext_boxplots){
    # Boxplots only for ktau, beta, and gamma, present everywhere
    ...
    for (k in seq_len(simulation_size)){
      reduced_para_est_mat[k,] <- all_para_estimates[[k]][
        seq_len(ktau_length_test + gamma_index)]
    }
    
    
    for (j in seq_len(ktau_length_test + gamma_index)){
...
external_print_fun_hom <- function(){
...
if (true_distr_T == "norm"){
  ...
  true_lambda <- "" # not actually used, but sometimes supplied as dummy variable
}

if (true_distr_T == "expWb"){
  # Note: true_wb_shape and true_wb_scale are left out for compatibility with
  # the other cases in the printing; these parameters are never changed
  # (so are true_betas and true_gammas), so they can just as well be defined
  # only once, globally.
  true_paras_T <- c(true_beta_0, true_beta_T, true_gam_par)
  true_transf_paras_T <- c(true_beta_0, true_beta_T, true_gam_par)
  true_paras_T_reduced_string <- paste0("shape = ", toString(true_wb_shape),
                                        ", scale = ", toString(true_wb_scale))
  true_lambda <- ""
}

# all excluding lambda (even in EAL scenario)
...
gamma_index <- X_dimension + 2 # only gamma_0 in homosk case
...
if (true_distr_T == "EAL"){
  qSurv_hom <- function(w,x){
    return(qEAL_hom(w, x, true_beta_full, true_gam_par, true_lambda, true_phi_til, true_phi))
  }
}

if (true_distr_T == "norm"){
  qSurv_hom <- function(w, x){
    s <- sigma_fun_hom(true_gam_par)
    return(x %*% true_beta_full + s * qnorm(w))
  }
}
...
if (true_distr_T == "expWb"){
  qSurv_hom <- function(w, x){
    s <- sigma_fun_hom(true_gam_par)
    return(x %*% true_beta_full + s * log(qweibull(w, shape = true_wb_shape, scale = true_wb_scale)))
  }
}
########################################################################
...
for (simulation_index in seq_len(simulation_size)){
  ...
  for (j in seq_len(sample_size)){
    T_on_X[j] <- qSurv_hom(cop_sample[j,1], X[j,])
  ...
  if (true_distr_T == "EAL"){
    if (true_cop_name == "indep"){
      true_llh <- log_lik_fun_indep_hom(true_transf_paras[-1], true_lag_degrees,
                                        lambda = true_lambda,
                                        true_cop_name, Y, X, Delta)
    } else {
      true_llh <- log_lik_fun_hom(true_transf_paras, true_lag_degrees,
                                  lambda = true_lambda, true_cop_name,
                                  Y, X, Delta)
    }
  ...
  intermed_res <- deg_sel_AIC_grid_only_T_hom(cop_name = test_cop_name,
                                              Y = Y, X = X, Delta = Delta,
                                              T1, X1, C0, X0,
                                              lambda = test_lambda,
                                              indep_assumption =
  ...
  mll_res <- perform_mll_optimisation_hom(test_lag_degrees, test_lambda,
                                          test_cop_name, Y, X, Delta,
  ...
  est_para_vec <- para_inv_transform_hom(est_transf_para_vec,
  ...
  for (qu_index in seq_along(quantile_levels)){
    ...
    est_quantile_fun <- function(x){
      return(qEAL_hom(w = qu_level, x = x, beta = est_beta_full,
                      gam_par = est_gam_par, lambda = test_lambda,
...
para_summary_stat <- para_performance_summary_hom(ext_print = TRUE,
                                                  ext_boxplots = TRUE)
